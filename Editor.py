...
class TextEditorApp:
    ...
    def Errores(self):
        ...
        for error in lista_errores:
            cont = 1
            er = error.operar(cont) +";C:"+ str(error.operarC(cont)) +";F:"+str(error.operarf(cont))
            cont += 1
            print(er)
        estructura_datos = {"errores": []}

        for i, error in enumerate(lista_errores, start=1):
            estructura_error = {
                "No": i,
                "descripcion": {
                    "lexema": error.operar(cont),
                    "tipo": "error lexico",
                    "columna": error.operarC(cont),
                    "fila":error.operarf(cont)
                }
            }
        ...
        with open(nombre_archivo, 'w') as archivo:
            json.dump(estructura_datos, archivo, indent=3)
